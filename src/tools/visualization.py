...
import logging
import os
from src.tools.tag_intel import get_value_type
...
def generate_correlation_chart(
    ...
    **legacy_kwargs,
    ...
    """
    Generic entry‑point used by the orchestrator to create a dual‑axis correlation plot.

    Parameters
    ----------
    cause_tag : str
        Tag name that is hypothesised as the *cause*.
    effect_tag : str
        Tag name that is the downstream *effect*.
    start_time : str
        ISO‑8601 timestamp (UTC) for the beginning of the window.
    end_time : str
        ISO‑8601 timestamp (UTC) for the end of the window.
    output_path : str | None, optional
        Full path where the PNG should be written.  If omitted a
        file name safe for the OS is autogenerated inside ``charts/``.
    legacy_kwargs : dict
        Catch‑all so that older positional calls
        ``generate_correlation_chart(primary_df, candidate_df, ...)`` still work
        without breaking.
    """
    # ------------------------------------------------------------------ #
    #  Back‑compat shim – old code may still call the *old* signature.
    #  If the first argument is *not* a string we assume the legacy form
    #  generate_correlation_chart(primary_df, candidate_df, ...)
    # ------------------------------------------------------------------ #
    if not isinstance(cause_tag, str):
        return _generate_correlation_chart_legacy(
            cause_tag,          # actually primary_df
            effect_tag,         # actually candidate_df
            start_time,         # actually primary_tag
            end_time,           # actually candidate_tag
            output_path or legacy_kwargs.get("correlation_value", 0.0),
            **legacy_kwargs,
        )

    # Otherwise use the new orchestrator‑friendly signature.
    from src.tools.data_loader import load_data

    primary_df = load_data(cause_tag, start_time, end_time)
    candidate_df = load_data(effect_tag, start_time, end_time)
    correlation_value = legacy_kwargs.get("correlation_value", 0.0)
...
# Helper for legacy correlation chart plotting (original implementation)
def _generate_correlation_chart_legacy(
    ...
    primary_is_binary = get_value_type(primary_tag) == 'binary'
    candidate_is_binary = get_value_type(candidate_tag) == 'binary'
    
    # Plot primary metric on left axis
    color1 = '#2E86AB'
    ax1.set_xlabel('Time', fontsize=12)
    ax1.set_ylabel(primary_tag, color=color1, fontsize=12)
    line1 = ax1.plot(primary_df['Timestamp'], primary_df['Value'],  # Assumes primary_df['Value'] is raw
    ...
    line2 = ax2.plot(candidate_df['Timestamp'], candidate_df['Value'],  # Assumes candidate_df['Value'] is raw
                     color=color2, linewidth=2, label=candidate_tag)
    ax2.tick_params(axis='y', labelcolor=color2)
    if candidate_is_binary:
